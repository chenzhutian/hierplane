...
app.get('/api/tree', (req, res) => {
  ...
  fs.readFile(path.resolve(__dirname, 'data', file), 'utf-8', (err, data) => {
    if (err) {
...
app.post('/api/tree', (req, res) => {
  const { tree, file } = req.body
  fs.writeFile(path.resolve(__dirname, 'data', file), JSON.stringify(tree), err => console.error(err))
})
